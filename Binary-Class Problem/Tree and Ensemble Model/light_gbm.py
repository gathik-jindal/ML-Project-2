...
def run_lightgbm():
    """
    Runs a binary classification model using a LightGBM Classifier
    ...
    # LightGBM can handle categorical features directly, but since we are using 
    # a ColumnTransformer for consistency across models, we keep the OneHotEncoder.
    ...
    # 5b. Define the LightGBM Model
    # LightGBM uses a leaf-wise growth strategy (faster but prone to overfitting)
    # compared to XGBoost's depth-wise strategy.
    lgbm_model = LGBMClassifier(
        n_estimators=100,          # Number of boosting stages (trees)
        learning_rate=0.1,         # Step size shrinkage
        num_leaves=31,             # Max number of leaves in one tree (default is 31, a good starting point)
        objective='binary',        # Target objective for binary classification
        random_state=42,
    ...
    test_preds = lgbm_pipeline.predict(X_test_submit)
...
if __name__ == '__main__':
    # Add a try-except block to check if LightGBM is installed
    try:
        run_lightgbm()
    except ImportError:
        print("\n--------------------------------------------------------------------------------")
        print("ERROR: LightGBM library is not installed.")
        print("Please install it using: pip install lightgbm")
        print("--------------------------------------------------------------------------------")
        sys.exit(1)